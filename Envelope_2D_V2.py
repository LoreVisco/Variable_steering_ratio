...
date_str = datetime.now().strftime("_%y%m%d_%H%M")
...
ext_angle = delta_2_joint(extreme_steering_wheel_angle,beta,alpha)
...
for  height in plane_height:
    ...
    index = (np.abs(rack_x-first_midpoint_x)).argmin()
    schifo = rack_y[:index+1]
    schifo = schifo[::-1]
    schifo_index = schifo[0:].index(0)+1

    start_of_curve_index = len(schifo)-schifo_index
    end_of_curve_index = start_of_curve_index
...
cq.exporters.export(result,output_filename+date_str+'.STEP')
